...
def get_CvoR_trans(degrees=3):
    ...
    :math:`\\frac {Cv_{trans}} {R} = 
    \\frac {N} {2}` where N is the degrees of freedom
...
def get_CvoR_vib(vib_energies, Ts):
    ...
    :math:`\\frac {Cp_{vib}}{R} = \\sum_{i=1}^{n} \\bigg(
    \\frac {\\Theta_{V,i}}{2T}\\bigg)^2 \\frac {1}{\\big(
    sinh(\\frac {\\Theta_{V,i}}{2T})\\big)^2}`
