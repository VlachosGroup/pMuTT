...
from PyMuTT import get_molecular_weight, _pass_expected_arguments
from PyMuTT import constants as c
from PyMuTT.models.statmech import trans, rot, elec, vib, StatMech
...
class TestStatMech(unittest.TestCase):
    ...
    def test_get_CvoR(self):
        self.assertAlmostEqual(
            self.CO2_PyMuTT.get_CvoR(T=self.T0, V=self.V0), 
                2.9422622359004853)

    def test_get_CpoR(self):
        self.assertAlmostEqual(
            self.CO2_PyMuTT.get_CpoR(T=self.T0, V=self.V0), 
                3.9422622359004853)

    def test_get_UoRT(self):
        self.assertAlmostEqual(
            self.CO2_PyMuTT.get_UoRT(T=self.T0, V=self.V0), 
                -875.1095022368354)
    ...
    def test_get_AoRT(self):
        self.assertAlmostEqual(
            self.CO2_PyMuTT.get_AoRT(T=self.T0, V=self.V0), 
                -900.5899966269182)
