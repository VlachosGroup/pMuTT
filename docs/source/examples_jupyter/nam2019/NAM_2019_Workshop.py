...
# # 4. Exercise 1
# 
# Using `pmutt.constants`, calculate the dimensionless activation energy (H/RT) using the following information:
# - H = 0.5 eV
# - T = 77 F
...
# - from a statistical mechanical model (``from_statmech``)
# - from heat capacity, enthalpy and entropy data (``from_data``)
# 
# <img src="images/nasa_func.png" width=400>
...
# Empirical objects can be made directly using ``StatMech`` objects and the ``from_statmech`` method.

# In[10]:


H2_nasa = Nasa.from_statmech(name='H2',
                             T_low=200.,
                             T_high=3500.,
                             statmech_model=H2_statmech)
...
for species_data in ab_initio_data:
    single_nasa_species = Nasa.from_statmech(T_low=100.,
                                             T_high=1500.,
                                             **species_data)
...
# 
# <img src="images/reaction_func.png" width=800>
# 
# 
